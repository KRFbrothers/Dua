...
class _VoiceScreenState extends State<VoiceScreen> {
  ...
  Future<void> _init() async {
    ...
    _pulseBoost = await RemotePrefs.loadPulseBoost();
    ...
    if (!available) {
      _setReply('Speech recognition unavailable. Google voice services check karo.');
      return;
    }
  ...
  Future<void> _askOnline(String text) async {
    ...
    _setReply(answer);
    final spoken = answer.length > 280 ? '${answer.substring(0, 280)}…' : answer;
    await _speak(spoken);
  }
  ...
  Future<void> _shareTranscript() async {
    final heard = _user.isEmpty ? (_partial.isEmpty ? '…' : _partial) : _user;
    final text = 'Dua voice transcript\n\n'
        'Tum: $heard\n\n'
        'Dua: $_reply';
    await SharePlus.instance.share(
      ShareParams(
        text: text,
        subject: 'Dua voice transcript',
      ),
    );
  }

  Future<void> _captureVideoOrPhoto() async {
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Video call later — camera capture only for now.'),
    ...
    try {
      ...
      await showDialog<void>(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: SelectableText(
            path,
            style: const TextStyle(color: DuaColors.textSecondary, fontSize: 13),
          ),
          actions: [
            TextButton(
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Saved preview path: $path'),
            behavior: SnackBarBehavior.floating,
          ),
        );
      }
  ...
  Future<void> _togglePulse() async {
    final next = !_pulseBoost;
    setState(() => _pulseBoost = next);
    await RemotePrefs.savePulseBoost(next);
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(next ? 'Pulse ON' : 'Pulse OFF'),
        behavior: SnackBarBehavior.floating,
  ...
  Widget build(BuildContext context) {
    final heard = _partial.isNotEmpty ? _partial : (_user.isEmpty ? '…' : _user);
    return Scaffold(
      body: Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      Expanded(
                        flex: 2,
                        child: NeonWaveform(
                          active: _listening || _busy || _pulseBoost,
                          intense: _pulseBoost,
                        ),
                      ),
                      Expanded(
              ...
              Row(
                ...
                children: [
                  _Control(
                    ...
                    'Share',
                    () => unawaited(_shareTranscript()),
                  ),
                  _Control(
                    Icons.videocam_outlined,
                    'Video',
                    () => unawaited(_captureVideoOrPhoto()),
                  ),
                  _Mic(active: _listening, enabled: !_busy, onTap: _toggleMic),
                  _Control(
                    Icons.graphic_eq,
                    'Pulse',
                    () => unawaited(_togglePulse()),
                    active: _pulseBoost,
                  ),
    ...
    children: [
      InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: end
                  ...
                  : (active ? DuaColors.cyan : DuaColors.borderNeon),
              width: active ? 2 : 1,
            ),
            boxShadow: active
                ? [
          ...
          child: Icon(
            icon,
            color: active ? DuaColors.cyanSoft : Colors.white,
          ),
        ),
      ),
      ...
      Text(
        label,
        style: TextStyle(
          color: active ? DuaColors.cyanSoft : DuaColors.textMuted,
          fontSize: 11,
        ),
      ),
    ],
